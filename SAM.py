...
class SAM(object):
    ...
    def filter_data(self, div=1, downsample=0, genes=None, cells=None, min_expression=1, thresh=0.02, filter_genes=True):
        ...
        if downsample > 0:
            ...
            self.filtered_dataset = self.filtered_dataset.iloc[np.random.choice(
                np.arange(self.filtered_dataset.shape[0]), size=numcells, replace=False), :]
    ...
    def load_annotations(self, aname):
        ...
        if(ann.size != self.filtered_dataset.shape[0]):
            ann = ann.values.flatten()[np.where(
                np.in1d(self.dataset.index.values, self.filtered_dataset.index.values))[0]]
        else:
    ...
    def louvain_clustering(self, res=2):
        if (not self.analysis_performed):
            ...
            adata = anndata.AnnData(self.D, var={'genes': self.gene_names}, obs={
    ...
    def plot_correlated_genes(self, name, n_genes=5, **kwargs):
        ...
        **kwargs - keyword arguments in 'show_gene_expression' and 'scatter' are eligible
        ...
        if(name.size == 0):
            print(
                "Gene note found in the filtered dataset. Note that genes are case sensitive.")
            return
